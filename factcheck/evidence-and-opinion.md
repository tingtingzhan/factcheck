...
>- **Don't restate it as your own.** Down-weighting bars an opinion from carrying weight; it does not license repeating its content. Before each sentence, ask where the judgment came from: if from an opinion block, attribute it in-line to that party or cut it. Unattributed restatement republishes it at full weight in your voice, where no guard fires on it. Reaches paraphrase and arrangement — an analysis whose emphasis and ordering track an opinion block has adopted it. Where an opinion block is a point's only source, the finding is that the point is **unestablished**, not the opinion with a hedge attached. Where the record excludes a tier outright, exclusion reaches restatement too.
